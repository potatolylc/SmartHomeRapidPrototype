...
public class DeviceServiceImpl implements DeviceService {
	...
	public DeviceValue retrieveDeviceInfo(String userName, String userWifiSsid,
			String deviceName) throws DeviceNotExistException, UserNotExistException {
		// Check whether user exists
		boolean isUserExistFlag = this.userService.isUserExist(userName, userWifiSsid);
		if(!isUserExistFlag)
			throw new UserNotExistException();
		
		// If user exists, get device information
		int userSerialNum = this.userService.retrieveUserSerialNumByUserNameAndSsid(new UserValue(userName, userWifiSsid));
		DeviceValue deviceVal = null;
		deviceVal = this.deviceRepository.findByDeviceNameAndUserSerialNum(deviceName, userSerialNum);
		if(deviceVal == null)
			throw new DeviceNotExistException();
		return deviceVal;
	}
